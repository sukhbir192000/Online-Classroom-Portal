...
module.exports.assignmentSubmission=async function(req,res){
    try{
        let assignment=await (await AssignmentModel.findById(req.params.assignmentId)).populate('classSub.course')
        .populate('classSub.class')
        .populate('classSub.group')
        .populate('classSub.subGroup');;
        let submission=await AssignmentSubmissionModel.find({
